...
function money(x: number): string {
  return 'RM' + (Math.round(x * 100) / 100).toLocaleString('en-MY', { minimumFractionDigits: 2 });
}
...
export function newExplanation(): Explanation {
  return {
    ...
    calc(step) {
      this.calcSteps.push(step);
    },
    firedSequence() {
      return this.cycles.map((c) => c.fired);
    },
    why() {
      return this.cycles.filter((c) => Object.keys(c.asserted).length).map((c) => ({
        rule: c.fired,
        text: c.because,
        facts: c.asserted,
      }));
  ...
  return [
    {
      ...
      act: (wm) => {
        wm.assertFact("eligible", true, "R001");
      },
...

    {
      ...
      salience: 95,
      cond: (wm) =>
        wm.value("zakatType") === "pendapatan" &&
        wm.value("halal") === false &&
        !wm.has("conclusion"),
      act: (wm) => {
...

    {
      ...
      salience: 95,
      cond: (wm) =>
        ["simpanan", "emas", "asb"].includes(String(wm.value("zakatType"))) &&
        wm.value("haul") === false &&
        !wm.has("conclusion"),
      act: (wm) => {
...

    {
      ...
      cond: (wm) =>
        ...
        wm.value("zakatType") === "pendapatan" &&
        wm.isTrue("halal") &&
        !wm.has("incomeAdmissible") &&
        !wm.has("conclusion"),
      act: (wm) => {
        wm.assertFact("incomeAdmissible", true, "R005");
      },
...

    {
      ...
      cond: (wm) =>
        wm.isTrue("eligible") &&
        wm.value("zakatType") === "simpanan" &&
        wm.isTrue("haul") &&
        !wm.has("payableBase") &&
        !wm.has("conclusion"),
      act: (wm, expl) => {
        const val = Calc.savingsAssessable(
          (wm.value("accounts", []) as SavingsAccount[]) || [],
          expl
        );
...

    {
      ...
      cond: (wm) =>
        wm.isTrue("eligible") &&
        wm.value("zakatType") === "emas" &&
        wm.isTrue("haul") &&
        hasGoldKat(wm, "perhiasan") &&
        !wm.has("goldWornZ") &&
        !wm.has("conclusion"),
      act: (wm, expl) => {
        wm.assertFact(
          "goldWornZ",
          round2(Calc.goldWorn((wm.value("goldItems", []) as GoldItem[]) || [], expl)),
          "R008"
        );
      },
...

    {
      ...
      cond: (wm) =>
        ...
        wm.value("zakatType") === "emas" &&
        wm.isTrue("haul") &&
        hasGoldKat(wm, "simpanan") &&
        !wm.has("goldStoredZ") &&
        !wm.has("conclusion"),
      act: (wm, expl) => {
        wm.assertFact(
          "goldStoredZ",
          round2(Calc.goldStored((wm.value("goldItems", []) as GoldItem[]) || [], expl)),
          "R009"
        );
      },
...

    {
      ...
      act: (wm, expl) => {
        wm.assertFact(
          "goldPawnedZ",
          round2(Calc.goldPawned((wm.value("goldItems", []) as GoldItem[]) || [], expl)),
          "R010"
        );
      },
...

    {
      ...
      cond: (wm) =>
        wm.value("zakatType") === "emas" &&
        wm.isTrue("haul") &&
        (wm.has("goldWornZ") || wm.has("goldStoredZ") || wm.has("goldPawnedZ")) &&
        !wm.has("payableBase") &&
        !wm.has("conclusion"),
      act: (wm, expl) => {
        const parts = [
          wm.value("goldWornZ", 0) as number,
          wm.value("goldStoredZ", 0) as number,
          wm.value("goldPawnedZ", 0) as number,
        ];
...

    {
      ...
      cond: (wm) =>
        wm.isTrue("eligible") &&
        wm.value("zakatType") === "asb" &&
        (wm.isTrue("haul") || wm.value("kelasAsb") === "B") &&
        !wm.has("payableBase") &&
        !wm.has("conclusion"),
      act: (wm, expl) => {
        const div = (wm.value("asbDiv") as number) || 0;

        // al-Mustaghallat: zakat falls on the YIELD only
        wm.assertFact("assessableAmount", round2(div), "R012");
        wm.assertFact("payableBase", round2(div), "R012");
        wm.assertFact("applicableRate", PARAMS.RATE_ASB, "R012");

        if (wm.value("kelasAsb") === "B") {
          // Khultah: entire ASB fund is one pooled estate that always exceeds nisab
          expl.calc(
            "Kelas B (khultah): nisab dinilai kolektif atas keseluruhan dana ASB, bukan baki individu."
          );
          wm.assertFact("nisabBasis", "khultah", "R012");
          wm.assertFact("nisabSatisfied", true, "R012");
        } else {
          // Kelas A: nisab tested on modal + dividen
          const modal = (wm.value("asbNilai") as number) || 0;
          const jumlah = round2(modal + div);
          expl.calc(
            `Semakan nisab = modal ${money(modal)} + dividen ${money(div)} = ${money(jumlah)}`
          );
          wm.assertFact("nisabTestValue", jumlah, "R012");
          wm.assertFact("nisabBasis", "rm", "R012");
        }
      },
      desc: "ASB al-Mustaghallat: zakat atas dividen × 2.57%, modal dikecualikan. Kelas A: nisab atas modal + dividen. Kelas B: nisab kolektif (khultah).",
      ref: "Muzakarah MKI 4 Sep 2025; Fatwa Selangor; ASNB",
    },

    {
      ...
      salience: 60,
      cond: (wm) =>
        wm.has("nisabTestValue") &&
        wm.value("nisabBasis") === "rm" &&
        (wm.value("nisabTestValue") as number) >= PARAMS.NISAB_RM &&
        !wm.has("nisabSatisfied") &&
        !wm.has("conclusion"),
      act: (wm) => {
        wm.assertFact("nisabSatisfied", true, "R014");
      },
...

    {
      ...
      salience: 60,
      cond: (wm) =>
        wm.value("nisabBasis") === "gold" &&
        (wm.value("payableBase", 0) as number) > 0 &&
        !wm.has("nisabSatisfied") &&
        !wm.has("conclusion"),
      act: (wm) => {
        wm.assertFact("nisabSatisfied", true, "R016");
      },
...

    {
      ...
      cond: (wm) =>
        wm.isTrue("eligible") &&
        wm.isTrue("nisabSatisfied") &&
        !wm.has("zakatObligatory") &&
        !wm.has("conclusion"),
      act: (wm) => {
        wm.assertFact("zakatObligatory", true, "R018");
      },
...

    {
      ...
      salience: 40,
      cond: (wm) =>
        wm.isTrue("zakatObligatory") && !wm.has("payableAmount"),
      act: (wm, expl) => {
        const z = Calc.payable(
          wm.value("payableBase"),
          wm.value("applicableRate") as number | undefined,
          expl
        );
...

    {
      ...
      cond: (wm) =>
        wm.value("zakatType") === "pendapatan" &&
        wm.has("payableAmount") &&
        !wm.has("monthly"),
      act: (wm) => {
        wm.assertFact(
          "monthly",
          round2((wm.value("payableAmount") as number) / 12),
          "R020"
        );
      },
