...
export interface Option  { v: unknown; l: string; sub?: string; }
export interface Field   { k: string; l?: string; unit?: string; }
...
export const Consult = {
  next(f: Facts): Question | null {
    ...
    if (!('muslim' in f))
      return Q('muslim', 'choice', 'Adakah anda seorang Muslim?', 'Syarat Wajib', {
        ...
        options: [{ v: true, l: 'Ya, saya Muslim' }, { v: false, l: 'Tidak' }],
      });
  ...
  _income(f: Facts): Question | null {
    if (!('halal' in f))
      return Q('halal', 'choice', 'Adakah sumber pendapatan anda halal?', 'Syarat Wajib', {
        ...
        options: [{ v: true, l: 'Ya, halal' }, { v: false, l: 'Tidak / sumber haram' }],
      });
    if (!('method' in f))
  ...
  _savings(f: Facts): Question | null {
    if (!('haul' in f))
      return Q('haul', 'choice', 'Adakah simpanan ini telah cukup haul?', 'Syarat Wajib', {
        ...
        options: [{ v: true, l: 'Ya, lebih setahun' }, { v: false, l: 'Tidak' }],
      });
  ...
  _asb(f: Facts): Question | null {
    if (!('haul' in f))
      return Q('haul', 'choice', 'Pelaburan ASB cukup haul?', 'Syarat Wajib', {
        options: [{ v: true, l: 'Ya' }, { v: false, l: 'Tidak' }],
      });
    if (!('kaedahAsb' in f))
      return Q('kaedahAsb', 'choice', 'Kaedah pengiraan zakat ASB?', 'Kaedah', {
        help: 'al-Mustaghallat (Fatwa Selangor 2025): zakat atas dividen sahaja, 2.57%.',
        options: [
          { v: 'tradisional',  l: 'Tradisional',     sub: 'Baki terendah × 2.5%' },
          { v: 'mustaghallat', l: 'al-Mustaghallat', sub: 'Dividen × 2.57%' },
        ],
      });
    if (f.kaedahAsb === 'tradisional' && !('asbBase' in f))
      return Q('asbBase', 'number', 'Baki terendah pelaburan (modal + dividen) setahun?', 'Maklumat', {
        field: { k: 'asbBase', unit: 'RM' },
      });
    if (f.kaedahAsb === 'mustaghallat' && !('asbNilai' in f))
      return Q('asbMust', 'group', 'Maklumat pelaburan & dividen ASB', 'Maklumat', {
        fields: [
          { k: 'asbNilai', l: 'Nilai pelaburan ASB semasa', unit: 'RM' },
          { k: 'asbDiv',   l: 'Dividen diterima setahun',   unit: 'RM' },
        ],
      });
    return null;
  },
  ...
  apply(f: Facts, qid: string, value: unknown): Facts {
    ...
    if (qid === 'accMore') {
      ...
      (f.accounts as unknown[]).push({ type: f._accType, baki: num(f._accBaki), faedah: num(f._accFaedah) });
      delete f._accType; delete f._accBaki; delete f._accFaedah;
      if (value === 'tidak') f._sDone = true;
      return f;
    }
    if (qid === 'gMore') {
      f.goldItems = (f.goldItems as unknown[]) ?? [];
      (f.goldItems as unknown[]).push({ kat: f._gKat, berat: num(f._gBerat), karat: f._gKarat, pinjaman: num(f._gPinjaman), upah: num(f._gUpah) });
      delete f._gKat; delete f._gBerat; delete f._gKarat; delete f._gPinjaman; delete f._gUpah;
    ...
    const keymap: Record<string, string> = {
      muslim: 'muslim', halal: 'halal', method: 'method', haul: 'haul',
      accType: '_accType', accBaki: '_accBaki', accFaedah: '_accFaedah',
      gKat: '_gKat', gBerat: '_gBerat', gKarat: '_gKarat',
      kaedahAsb: 'kaedahAsb', asbBase: 'asbBase',
    };
    const numericIds = new Set(['accBaki','accFaedah','gBerat','asbBase']);
